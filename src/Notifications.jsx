...
const Notifications = ({ isVisible, toggleVisibility }) => {
  ...
  return (
    <section
      ref={sectionRef}
      className={`fixed top-0 right-0 h-screen w-[420px] bg-white shadow-lg transition-transform duration-300 ease-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}
    >
        ...
        <ul className="flex-1 overflow-y-auto">
          {Array(2).fill().map((_, index) => (
            <li key={index} className="flex items-start px-6 py-4 border-b border-gray-200 hover:bg-purple-50">
              <div className="bg-purple-100 text-sky-600 rounded-full h-8 w-8 flex items-center justify-center mr-4">
