...
const OptionFilter = ({
  ...
  const getCount = () => includedOptions[optionType].length;
            ...
            {getCount() > 0 && (
              <span className='counter-country border border-gray-300 rounded-sm h-5 mr-2 text-gray-500'>
                <span className='flex items-center ml-1'>
                  ...
                  <X size={14} className="ml-1 cursor-pointer" onClick={() => onClearAllOptions()} />
                </span>
              </span>
            )}
          ...
          <div className="included text-xs font-normal pl-4 pb-4 bg-white">
            ...
            <div className="flex flex-wrap">
              {includedOptions[optionType].map((option) => (
                ...
                >
                  <span>{option}</span>
                  <CircleOff size={13} className="ml-1 cursor-pointer" onClick={() => onToggleOption(optionType, option, false)} />
          ...
          <div className="excluded text-xs font-normal pl-4 pb-4 bg-white">
            ...
            <div className="flex flex-wrap">
              {excludedOptions[optionType].map((option) => (
                ...
                >
                  <span>{option}</span>
                  <Plus size={13} className="ml-1 cursor-pointer" onClick={() => onToggleOption(optionType, option, true)} />
                  <X size={13} className="ml-1 cursor-pointer" onClick={() => {
                    onRemoveOption(optionType, option);
                    onToggleVisibility(optionType);
                  }} />
