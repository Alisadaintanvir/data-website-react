...
const Navbar = ({ onMenuClick }) => { // Accept onMenuClick as a prop
  ...
  return (
    <nav className="w-full h-12 flex justify-between items-center py-0 px-4 bg-[#1c2548] text-blue-400 sticky top-0">
      <div className="flex items-center">
        <div className="text-2xl font-bold xl:ml-4 mr-2 lg:mr-4 cursor-pointer" onClick={() => onMenuClick('dashboard')} >LOGO</div>
        <div
              ...
              <li key={href} className="my-2 lg:my-0 text-[14px]">
                <a
                  ...
                  className={`block px-4 py-3 rounded-sm transition-colors duration-300 ${
                    activeLink === href ? 'text-white bg-[#243759] lg:border-b-4 lg:border-b-blue-500' : 'text-slate-300'
                  }`}
                  onClick={() => {
                    setActiveLink(href);
                    setIsMenuOpen(false);
                    onMenuClick(label.toLowerCase()); // Call onMenuClick with the component name
                  }}
                >
                  {label}
                </a>
      ...
      <span className={`${
        ...
        <Bell size={23} className="cursor-pointer text-white" onClick={() => onMenuClick('notifications')} />
        <Settings size={23} className="cursor-pointer text-white" onClick={() => onMenuClick('settings')} /> {/* Handle settings click */}
        <CircleUser size={23} className="cursor-pointer text-white" onClick={() => onMenuClick('profile')} />
      </div>
    </nav>
  );
};
